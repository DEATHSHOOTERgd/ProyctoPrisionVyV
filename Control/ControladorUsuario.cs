...
namespace Control {
  ...
  /// <summary>
  /// Controlador que gestiona la informaciÛn de <see cref="Usuario">.
  /// </summary>
  public class ControladorUsuario {
    /// <summary>
    /// Instancia de <see cref="Rol"/> sirve como auxiliar para los mÈtodos de control.
    /// </summary>
    public Rol role = null;
    /// <summary>
    /// Atributo tipo <see cref="DatosLogin"/> que permite el acceso a la capa de Datos.
    /// </summary>
    private DatosLogin _datosLogin = new DatosLogin();
    /// <summary>
    /// Instancia de <see cref="Usuario"/> sirve como auxiliar para los mÈtodos de control.
    /// </summary>
    ...
    /// <param name="contrasena">ContraseÒa ingresada.</param>
    ...
    /// </summary>
    /// <returns>Una lista con la informaciÛn de cada <see cref="Usuario"/> registrado en el sistema.</returns>
    /// <exception cref="GeneralExcepcion">Cuando no se encontrÅEning˙n <see cref="Usuario"/> registrado en el sistema.</exception>
    public List<Object> ListarUsuarios() {
      List<Usuario> usuarios = _datosLogin.Consultar();
    ...
    public Object BuscarUsuario(string user) {
      return ConvertirAnonimo(_datosLogin.ConsultarUsuario(user));
    }
    /// <summary>
    /// Actualiza los datos de un <see cref="Usuario"/> en espec˙Éico.
    /// </summary>
    ...
    /// <param name="contrasena">Nueva contraseÒa del <see cref="Usuario"/>.</param>
    /// <param name="rol">Nuevo <see cref="Rol"/> del <see cref="Usuario"/>.</param>
    /// <returns>True si se actualizÅEcorrectamente, caso contrario devuelver false.</returns>
    public bool ActualizarUsuario(string nombre, string apellido, string usuario, string contrasena, string rol) {
      role = new Rol(rol);
      _user = new Usuario(nombre, apellido, usuario, contrasena, ObtenerRol(rol));

      try {
        _datosLogin.ActualizarUsuario(_user);
        return true;
    ...
    /// <summary>
    /// Busca un el rol de un usuario espec˙Éico que coincida con el <paramref name="usuario"/> ingresado.
    /// </summary>
    /// <param name="usuario">Nombre de usuario del <see cref="Usuario"/> a buscar.</param>
    /// <returns>El <see cref="Rol"/> del <see cref="Usuario"/> espec˙Éico.</returns>
    public string RetornaRol(string usuario) {
      _user = null;
      _user = _datosLogin.ConsultarUsuario(usuario);
      return _user.Rol.Descripcion;
    }
    /// <summary>
    ...
    /// <param name="contrasena">ContraseÒa del nuevo <see cref="Usuario"/>.</param>
    /// <param name="rol">Rol del nuevo <see cref="Usuario"/>.</param>
    /// <returns>True si se insertÅEcorrectamente, caso contrario devuelver false.</returns>
    public bool GuardarUsuario(string nombre, string apellido, string usuario, string contrasena, string rol) {
      role = new Rol(rol);
      _user = new Usuario(nombre, apellido, usuario, contrasena, ObtenerRol(rol));

      try {
        _datosLogin.InsertarUsuario(_user);
        return true;
      } catch (ConsultaFallida) {
    ...
    /// <summary>
    /// Busca un rol cuya descripciÛn coincida con <paramref name="rol"/>.
    /// </summary>
    /// <param name="rol"></param>
    /// <returns>Un <see cref="Rol"/>.</returns>
    public Rol ObtenerRol(string rol) {
      return _datosLogin.ConsultarRol(rol);
    }
    /// <summary>
    /// Convierte una lista de <seealso cref=Usuario"/>s en una lista de objetos anÛnimos(<seealso cref="Object"/>) que la capa de vista pueda entender.
    /// </summary>
    /// <param name="usuarios">Lista de usuarios a convertir.</param>
    /// <returns>Una lista de objetos anÛnimos(<seealso cref="Object"/>).</returns>
    ...
    /// <summary>
    /// Convierte un objeto <seealso cref="Usuario"/> en un objeto anÛnimo(<seealso cref="Object"/>) que la capa de Vista puede entender.
    /// </summary>
